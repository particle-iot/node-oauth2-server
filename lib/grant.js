 ...
 * @param  {Function} done
 */
function mfaOtp (done) {
  var self = this;
  if (self.req.body) {
    if (!self.req.body.otp || !self.req.body.mfa_token) {
  ...
  this.model.mfaOtp(this.req,
    function (err, user) {
      if (err) {
          return done(error(err.error, err.message, err ));
      }
      ...
      if (self.req.body && user.expires_at) {
